...
@ignore_warnings(category=ConvergenceWarning) # TODO: Can we get rid of this?
def evaluate_all_lifetimes(X, y, X_test, y_test, M, lifetime_max, delta,
                           validation=False, uniformly_rotated=False, mondrian_kernel=False, mondrian_forest=False, 
                           weights_from_lifetime=None):
    """
    ...
    # duplicate data per features for data manipulation, e.g., uniform rotation
    if(not uniformly_rotated):
        X_all_dup = np.repeat(X_all[None,:], M, axis=0)
    # uniform rotation
    else:
        X_all_average = np.tile(np.average(X_all, axis=0), N_all).reshape(N_all, D)
        X_all_dup = np.array([(X_all - X_all_average) @ scipy.stats.special_ortho_group.rvs(dim=D) + X_all_average for _ in range(M)])

    # formatting for mondrian kernel and mondrian forest
    ...
    indptr = [i for i in range(0, M * N_all + 1, M)]
    indices = [(i % M) for i in range(M * N_all)]
    data = np.ones(N_all * M) / np.sqrt(M)
    Z_all = scipy.sparse.csr_matrix((data, indices, indptr), shape=(N_all, M))
    feature_from_repetition = [i for i in range(M)]
    C = M

    # bounding box for all datapoints used to sample first cut in each tree
    feature_data = [np.array(range(N_all)) for _ in range(M)]
    lX = np.array([np.min(X_all_dup[i], 0) for i in range(M)])
    uX = np.array([np.max(X_all_dup[i], 0) for i in range(M)])

    # event = tuple (time, feature, tree, dim, loc), where feature is the index of feature being split
    events = []
    active_features = []
    active_features_in_tree = [[] for _ in range(M)]
    for m in range(M):
        cut_time, dim, loc = sample_cut(lX[m], uX[m], 0.0)
    ...
    while len(events) > 0:
        ...
        Xd = X_all_dup[m][feature_data[c], dim]
        ...
        lX_l = np.min(X_all_dup[m][feature_l, :], axis=0)
        uX_l = np.max(X_all_dup[m][feature_l, :], axis=0)
        cut_time_l, dim_l, loc_l = sample_cut(lX_l, uX_l, birth_time)
        lX_r = np.min(X_all_dup[m][feature_r, :], axis=0)
        uX_r = np.max(X_all_dup[m][feature_r, :], axis=0)
        ...
        if mondrian_kernel:
            ...
            SGD_epochs = 1
            clf = linear_model.SGDRegressor(alpha=delta, fit_intercept=False, max_iter=SGD_epochs)
            ...
            if validation:
                error_train, error_validation =\
                    errors_regression(y, y_test[:int(N_test/2)], y_hat_train, y_hat_test[:int(N_test/2)])
...
def uniformly_rotated_Mondrian_kernel_features(X, lifetime, M):
    res = evaluate_all_lifetimes(X, None, np.empty((0, X.shape[1])), None, M, lifetime, None, uniformly_rotated=True)
    Z = np.sqrt(M) * res['Z']   # undo normalization
    return Z, res['feature_from_repetition']