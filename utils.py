...
def construct_data_synthetic_uniformly_rotated(D, lifetime, noise_var, N_train, N_test):
    # pick datapoint locations uniformly at random
    N = N_train + N_test
    X = np.random.rand(N, D)
...
def select_width(X, y, X_test, y_test, M, delta, eval_function, num_iters):
    ...
    list_runtime = [time.process_time() - time_start]
    list_error_test = [error_test]
    ll_errors = [(lg_lifetime, error_test)]

    for i in range(num_iters):
        am = np.argmin(ll_errors, axis=0)[1] # OLD: np.argmin(zip(*ll_errors)[1])
...
def initialize_plotting():
    matplotlib.rc('text', usetex=True) # TODO: fix: RuntimeError: Failed to process string with tex because latex could not be found
    matplotlib.rc('font', family='Helvetica', size=14) # TODO: fix 'Font family 'Helvetica' not found.'
    return
