...
def evaluate_random_binning(X, y, X_test, y_test, M, lifetime, delta):
    ...
    Z = rb.get_features(X)[0] / np.sqrt(M)
    Z_test = rb.get_features(X_test, expand=False)[0] / np.sqrt(M)

    # solve primal problem using SGD
    SGD_epochs = 10
    error_test = SGD_regression_test_error(Z, y, Z_test, y_test, delta, SGD_epochs)
    sys.stdout.write('\rRB lg_lifetime = %.2f; C = %d; error_test = %.2f%%   ' % (np.log2(lifetime), np.shape(Z)[1], error_test))
    sys.stdout.flush()
    return error_test
